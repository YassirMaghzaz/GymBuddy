...
class PreferencesController < ApplicationController
  ...
  def create
    @preference = @profile.preferences.build(preference_params)
    if @preference.save
      redirect_to profile_path(@profile), notice: 'Preferences created successfully.'
    else
      render :new
    end
  end
  ...
  def preference_params
    params.require(:preference).permit(:workout_days, :workout_type, :gym_id)
  end
end
